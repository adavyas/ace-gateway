...
def _image_ref(image_tag: Optional[str]) -> str:
    raw = (image_tag or "").strip()
    if raw and any(marker in raw for marker in ("/", ":", "@")):
        return raw
    tag = (raw or config.ACE_RUNTIME_IMAGE_TAG or "latest").strip()
...
def _required_volume_labels(identity: RuntimeIdentity) -> dict[str, str]:
    return {
        "ace.managed": "true",
        "ace.user_id": identity.user_id,
    }


def _required_container_labels(identity: RuntimeIdentity) -> dict[str, str]:
    return {
        "ace.managed": "true",
        "ace.user_id": identity.user_id,
    }
...
async def _run_local(cmd: str, *, timeout_seconds: float, check: bool = True) -> tuple[int, str, str]:
    ...
    try:
        ...
        proc.kill()
        raise RuntimeProvisioningError(
            f"Local command timed out after {timeout_seconds:.1f}s",
            status_code=500,
        ) from exc
    ...
    if check and rc != 0:
        msg = stderr or stdout or f"command failed with exit code {rc}"
...
async def _ensure_network_exists() -> None:
    ...
    rc, _, _ = await _run_local(inspect_cmd, timeout_seconds=10.0, check=False)
    if rc == 0:
        return
    create_cmd = _docker_cmd("docker", "network", "create", network)
    await _run_local(create_cmd, timeout_seconds=15.0, check=True)


async def _inspect_volume(volume_name: str) -> Optional[dict]:
    inspect_cmd = _docker_cmd("docker", "volume", "inspect", volume_name)
    rc, stdout, _ = await _run_local(inspect_cmd, timeout_seconds=10.0, check=False)
    if rc != 0:
        return None
    try:
        ...
        raise RuntimeProvisioningError(
            f"Unable to inspect docker volume {volume_name}",
            status_code=500,
        ) from exc
    if not payload:
        return None
...
def _assert_volume_matches(identity: RuntimeIdentity, volume: dict) -> None:
    labels = volume.get("Labels") or {}
    for key, expected in _required_volume_labels(identity).items():
        actual = str(labels.get(key) or "")
        if actual != expected:
            raise RuntimeProvisioningError(
                f"Volume {identity.volume_name} failed identity check for {key}",
                status_code=409,
            )


async def _ensure_volume_exists(identity: RuntimeIdentity) -> None:
    volume = await _inspect_volume(identity.volume_name)
    if volume is not None:
        _assert_volume_matches(identity, volume)
        return
...
async def _inspect_container(container_name: str) -> Optional[dict]:
    cmd = _docker_cmd("docker", "container", "inspect", container_name)
    rc, stdout, _ = await _run_local(cmd, timeout_seconds=10.0, check=False)
    if rc != 0:
        return None
    try:
        payload = json.loads(stdout or "[]")
        ...
        raise RuntimeProvisioningError(
            f"Unable to inspect docker container {container_name}",
            status_code=500,
        ) from exc
    if not payload:
        return None
...
def _assert_container_matches(identity: RuntimeIdentity, container: dict, *, expected_image_ref: Optional[str]) -> None:
    config_data = container.get("Config") or {}
    labels = config_data.get("Labels") or {}
    for key, expected in _required_container_labels(identity).items():
        actual = str(labels.get(key) or "")
        if actual != expected:
            raise RuntimeProvisioningError(
                f"Container {identity.container_name} failed identity check for {key}",
                status_code=409,
            )

    if expected_image_ref:
        actual_image = str(config_data.get("Image") or "").strip()
        if actual_image != expected_image_ref:
            raise RuntimeProvisioningError(
                f"Container {identity.container_name} is running {actual_image or 'unknown image'}, expected {expected_image_ref}",
                status_code=409,
            )

    networks = ((container.get("NetworkSettings") or {}).get("Networks") or {})
    if config.ACE_DOCKER_NETWORK not in networks:
        raise RuntimeProvisioningError(
            f"Container {identity.container_name} is not attached to docker network {config.ACE_DOCKER_NETWORK}",
            status_code=409,
        )

    mounts = container.get("Mounts") or []
    expected_dest = config.ACE_RUNTIME_DATA_MOUNT
    volume_ok = any(
        str(mount.get("Type") or "") == "volume"
        and str(mount.get("Name") or "") == identity.volume_name
        and str(mount.get("Destination") or "") == expected_dest
        for mount in mounts
    )
    if not volume_ok:
        raise RuntimeProvisioningError(
            f"Container {identity.container_name} is not bound to expected volume {identity.volume_name}",
            status_code=409,
        )
...
async def _start_container(container_name: str) -> None:
    ...
    await _run_local(cmd, timeout_seconds=20.0, check=True)
...
async def _ensure_connected_to_network(container_name: str) -> None:
    ...
    rc, stdout, stderr = await _run_local(cmd, timeout_seconds=10.0, check=False)
    if rc == 0:
        return
    joined = f"{stdout}\n{stderr}".lower()
...
async def _create_container(identity: RuntimeIdentity, *, options: RuntimeOptions, image_ref: str) -> None:
    ...
    if config.ACE_GLOBAL_SKILLS_DIR and os.path.exists(config.ACE_GLOBAL_SKILLS_DIR):
        run_args.extend(
            [
                "-v",
    ...
    await _run_local(cmd, timeout_seconds=45.0, check=True)
...
async def _probe_inside_container(container_name: str) -> tuple[bool, str]:
    ...
    rc, stdout, stderr = await _run_local(cmd, timeout_seconds=6.0, check=False)
    if rc == 0:
        return True, ""
...
async def _container_logs_tail(container_name: str, *, lines: int) -> str:
    ...
    _, stdout, stderr = await _run_local(cmd, timeout_seconds=10.0, check=False)
    ...
    *,
    ...
    async with lock:
        ...
        if status is None:
            ...
            container = await _inspect_container(identity.container_name)
            if container is None:
                raise RuntimeProvisioningError(
                    f"Container {identity.container_name} disappeared during inspection",
                    status_code=500,
                )
            _assert_container_matches(
                identity,
                container,
                expected_image_ref=(image_ref if opts.image_tag else None),
            )
            if status != "running":
