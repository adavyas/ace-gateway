...
app = FastAPI(title="ace-fake-agent")
...
async def readyz():
    ...
    ok_calls = int(os.getenv("ACE_READY_OK_CALLS", "0"))
    if ok_calls > 0 and call_no > ok_calls:
        return JSONResponse(status_code=503, content={"status": "not_ready", "reason": "ready_calls_exceeded"})

    return {"status": "ready", "ready_calls": call_no}
