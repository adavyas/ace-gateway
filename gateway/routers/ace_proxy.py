...
async def ace_health() -> dict[str, Any]:
    target_url = f"{config.ACE_UPSTREAM_BASE_URL}/"
    try:
...
async def proxy_ace_path(
    ...
    target_url = f"{runtime.upstream_url}/{path.lstrip('/')}"
    ...
    if wants_sse:
        async def _stream() -> AsyncIterator[bytes]:
            try:
                async with httpx.AsyncClient(timeout=timeout) as client:
                    async with client.stream(
                        ...
                        if upstream.status_code >= 400:
                            yield _sse_error(f"ACE upstream returned HTTP {upstream.status_code}")
                            return
                ...
                logger.exception("ACE SSE proxy failed for %s", target_url)
    ...
    try:
        ...
        logger.exception("ACE proxy failed for %s", target_url)
